...
function showNotification(extensionStatusJSON) {
  ...
  if (extensionStatusJSON.status == 200) {
    obj.style.cssText = `color: #66f600; ${commonCSS}`
    ...
    mutationsList.forEach((mutation) => {
      if (
        ...
        const currentPersonName = person.childNodes[0]
          ? person.childNodes[0].textContent
          : ""
        ...
        if (beforeTranscriptText == "") {
          ...
          if (personNameBuffer != currentPersonName) {
            ...
            transcriptTextBuffer += currentTranscriptText.substring(
              currentTranscriptText.indexOf(beforeTranscriptText) +
                beforeTranscriptText.length
            )
            beforeTranscriptText = currentTranscriptText
          }
        }
